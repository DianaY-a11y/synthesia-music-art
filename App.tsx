...
const SCAN_COLUMNS = 512; // High Resolution for longer/more detailed songs
const SCAN_ROWS = 32; 
...
function App() {
  ...
  const extractImageGrid = (src: string) => {
    ...
    img.onload = () => {
      ...
      dynamicWidth = Math.max(512, Math.min(2048, dynamicWidth));
  ...
  useEffect(() => {
    if (analysis && selectedInstrument) {
      const newParams = { ...analysis.audioParams, instrument: selectedInstrument as any };
      setAnalysis({ ...analysis, audioParams: newParams });
  ...
  const animate = (time: number) => {
    ...
    if (state === AppState.PLAYING && analysis) {
      ...
      const totalDuration = beatDuration * (currentScanWidth / 8); 
      ...
      const colIndex = Math.floor(scanPos * currentScanWidth) % currentScanWidth;
  ...
  const playColumn = (colIndex: number) => {
    const column = gridDataRef.current[colIndex];
    if (!column || !analysis) return;

    const threshold = 0.2; 
    let activeCells = column.filter(c => c.brightness > threshold);

    if (activeCells.length === 0) return;
    
    const density = analysis.audioParams.density;

    // STRATEGY A: POLYPHONIC
    // Good for: Pluck, Synth, Chime, 8-bit
    // These instruments decay relatively fast or sound good in chords.
    // We want to hear the "spread" of the visual texture.
    const polyphonicInstruments = ['pluck', 'synth', 'chime', '8-bit'];
    
    if (polyphonicInstruments.includes(selectedInstrument)) {
      // Sort by brightness descending
      activeCells.sort((a, b) => b.brightness - a.brightness);
      
      // Play Top N brightest
      const maxPolyphony = 4;
      const topCells = activeCells.slice(0, maxPolyphony);

      topCells.forEach(cell => {
        if (Math.random() < density) {
          const yPos = cell.originalRowIndex / SCAN_ROWS;
          audioEngine.triggerGrain(yPos, cell.brightness, cell.r, cell.g, cell.b);
        }
      });
      return;
    }

    // STRATEGY B: MONOPHONIC / WEIGHTED AVERAGE
    // Good for: Violin, Pad, Drone, Bass
    // These are sustained, heavy sounds. Playing 4 of them at once creates mud.
    // We calculate the "Center of Mass" of the visual slice to play one coherent melody line.
    
    // FIX 2: RHYTHMIC THROTTLING (Legato Mode)
    // For sustained instruments like Violin, we don't want to retrigger the envelope
    // every single pixel column (which is extremely fast). 
    // We quantize triggers to every 8th column (approx 1 beat) to let the long attack/release breathe.
    if ((selectedInstrument === 'violin' || selectedInstrument === 'pad' || selectedInstrument === 'drone') && colIndex % 8 !== 0) {
      return;
    }
    ...
    // Calculate Averages
    const avgY = weightedYSum / totalBrightness;
    const avgBrightness = totalBrightness / activeCells.length;
    const avgR = rSum / totalBrightness;
    const avgG = gSum / totalBrightness;
    const avgB = bSum / totalBrightness;

    // Play the computed "Average" note
    // For throttled instruments, we increase probability to 1.0 (or high density) 
    // because we are already skipping 7/8ths of the opportunities.
    const prob = (selectedInstrument === 'violin' || selectedInstrument === 'pad') ? 1.0 : Math.max(density, 0.4);

    if (Math.random() <= prob) {
       audioEngine.triggerGrain(avgY, avgBrightness, avgR, avgG, avgB);
    }
  };
  ...
  }, [state, analysis, scanPos, userTempo, selectedInstrument]); 
    ...
    <div className="relative w-full h-screen bg-black text-white overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none flex flex-col justify-between p-6 z-10">
        ...
        {analysis && state !== AppState.ANALYZING && (
          <div className="pointer-events-auto flex flex-col md:flex-row gap-4 items-end animate-fade-in-up">
            ...
            {/* Info Card */}
            <Card className="flex-1 max-w-5xl backdrop-blur-xl bg-black/70 border-white/20">
              <div className="flex flex-col gap-4">
                
                {/* Top Row: Title & Params */}
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-gray-700 pb-4">
                  <div>
                    <h2 className="text-2xl font-serif font-bold text-transparent bg-clip-text bg-gradient-to-r from-white to-gray-400">
                  ...
                  <div className="flex flex-wrap gap-4 items-end">
                     <Select 
                       ...
                       options={['synth', 'pad', 'pluck', 'drone', 'chime', 'bass', '8-bit', 'violin']}
